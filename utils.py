...
def notify_user_about_reset_password(current_user,reset_url, tpl_name, config):
    user = current_user
    msg = render_template(tpl_name, user=user, 
    reset_url=reset_url, application_name=config.get('APP_NAME'))

    # logging.info(msg)

    subject = __('Your %(appName)s application reset password', appName = config.get('APP_NAME'))
    
    send_email_smtp(user.email, subject, msg, config ,
                    dryrun=not config.get('EMAIL_NOTIFICATIONS'))
