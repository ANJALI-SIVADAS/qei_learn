...
class CustomParamTypes(Model):

    _tablename_ = 'custom_param_types'
    id = Column(Integer, primary_key=True)
...
class CustomParameters(Model):
    
    """store parameter details"""

    _tablename_ = 'custom_parameters'
    id = Column(Integer, primary_key=True)
    param_name = Column(String(250), nullable=False)
    param_type_id = Column(Integer, ForeignKey('custom_param_types.id'), nullable=False)
    param_type = relationship('CustomParamTypes')
    default_param_value = Column(String(250), nullable=False)
    param_values = Column(Text, nullable=False)
    param_alias = Column(Text, nullable=True)
...
class Slice(Model, AuditMixinNullable, ImportMixin):
    ...
    def get_explore_url(self, base_url='/qei/explore', overrides=None):
    ...
    def explore_json_url(self):
        """Defines the url to access the slice"""
        return self.get_explore_url('/qei/explore_json')
            ...
            if default_filters and default_filters != '{}':
                try:
                    if json.loads(default_filters):
                        ...
                        return '/qei/dashboard/{}/?preselect_filters={}'.format(
                            self.slug or self.id, filters)
                except Exception:
                    pass
        return '/qei/dashboard/{}/'.format(self.slug or self.id)
    ...
    def roles_with_datasource(self):
        ...
        for r in pv.role:
            if r.name in self.ROLES_BLACKLIST:
                continue
            url = (
                '/qei/approve?datasource_type={self.datasource_type}&'
                'datasource_id={self.datasource_id}&'
