...
# Forgot password related constants
DATETIME_FORMAT = '%Y-%m-%d %H:%M:%S'
PRIVATE_KEY = b"DGeyn-ISH5jEi5zt-hInL13EqZpCyQHXnm6WxuRmF9I="
SEPERATION_QUOTE = "|"
TIME_INTERVAL = 15
INVALID_TOKEN = "INVALID_TOKEN"

def token_encryption(email_address,time_stamp):

    constructed_text = email_address + SEPERATION_QUOTE + time_stamp
    
    cipher_suite = Fernet(PRIVATE_KEY)
    cipher_text = cipher_suite.encrypt(bytes(constructed_text))

    return cipher_text

def token_decryption(content_string):
    try:
        cipher_suite = Fernet(PRIVATE_KEY)
        decrypted_string = cipher_suite.decrypt(bytes(content_string))
        metadata_array = decrypted_string.split(SEPERATION_QUOTE)
        
        # Create a dictionary with the meta info in the token
        token_metadata = {}
        token_metadata["email_address"] = metadata_array[0]
        token_metadata["time_stamp"] = metadata_array[1]
...
def timestamp_validation(timestamp):
    ...
    # Convert the string time stamp into datatime format
    timestamp = datetime.strptime(timestamp, DATETIME_FORMAT)
        
    # Get the difference time as mins
    diff_in_mins = (current_time - timestamp).total_seconds()/60
...
class QEI(BaseSupersetView):
    ...
    @expose("/forgotpassword/")
    def forgotpassword(self):
        # print(request.)
        return self.render_template(
            'superset/forgot_password.html'
        )

    @api
    @expose("/forgotpassword/<email_address>/", methods=['GET', 'POST'])
    # Forgot password reset API
    def forgotpassword_reset(self, email_address):
        # Check the email address is a valid one or not 
        if not re.match(r"^[A-Za-z0-9\.\+_-]+@[A-Za-z0-9\._-]+\.[a-zA-Z]*$", email_address):
            flash(__("Entered email address is not a valid email address, please enter a valid email address."), "warning")
            return redirect('/qei/forgotpassword/')
        
        # Load the user from database using email address 
        current_user = security_manager.find_user(email= email_address)
        print(current_user)
        # Check the given email address is in the database or not
        if not current_user:
            flash(__("Entered email address isn't attach with a user profile in the application"), "danger")
            return redirect('/qei/forgotpassword/')
        ...
        security_manager.update_user(current_user)

        dynamic_reset_url = request.url_root+'qei/resetpassword/token/'+ token_encryption(email_address,current_time)

        # Send the new password via email
        utils.notify_user_about_reset_password(current_user,dynamic_reset_url,
        'email/forgot_password.txt', app.config)

        success_message = __('An email with a reset password url has been sent to %(email)s. please check your inbox.', email = email_address)
        
        flash(__(success_message), "info")
        return redirect('/login')
    
    # API method to accept the dynamic password reset URL
    @api
    @expose("/resetpassword/token/<access_token>/", methods=['GET', 'POST'])
    # Forgot password reset API
    def resetpassword(self, access_token):
        access_info = token_decryption(access_token)
        
        if access_info == INVALID_TOKEN:
            flash(__("Password reset url is not a valid url."), "danger")
            return redirect('/login')

        email_address = access_info["email_address"]
        timestamp_string = access_info["time_stamp"]
        
        # Check the timestamp validity
        diff_in_mins = timestamp_validation(timestamp_string)
        
        # Check the timestamp interval 
        if diff_in_mins > TIME_INTERVAL:
            flash(__("Password reset url has been expired."), "danger")
            return redirect('/login')
    
        # Load the user from database using email address 
        current_user = security_manager.find_user(email= email_address)

        # Check the given email address is in the database
        if not current_user:
            flash(__("URL is not attach with a user profile in the application"), "danger")
            return redirect('/qei/forgotpassword/')
        
        last_loagin_timestamp = current_user.last_login.strftime(DATETIME_FORMAT)

        if timestamp_string != last_loagin_timestamp:
            flash(__("Password reset url has been expired."), "danger")
            return redirect('/login')

        # User will be routed to the reset password page if the URL is a valid one
        return self.render_template(
            'superset/reset_password.html',access_token=access_token,email_address = email_address
        )

    @log_this
    @api
    @expose("/savepassword/<access_token>/", methods=['GET', 'POST'])
    # Forgot password reset API
    def save_new_password(self, access_token):
        password_info = access_token.split("^")

        access_info = token_decryption(password_info[0])
        ...
        confirm_password = password_info[2]
        ...
        # random_password = ''.join(random.SystemRandom().choice(string.ascii_uppercase + string.digits) for _ in range(10))
        # Hash the password using existing salt 
        ...
        # Update the user along with the new password
        isSuccess = security_manager.update_user(current_user)
        
        flash(__("Password has been updated successfully!"), "info")
        return redirect('/login/')
    ...
    def request_access(self):
        ...
        if has_access:
            return redirect('/qei/dashboard/{}'.format(dashboard_id))
    ...
    def slice(self, slice_id):
        ...
        endpoint = '/qei/explore/?form_data={}'.format(
    ...
    def testconn(self):
        ...
        try:
            ...
            if database and uri:
                ...
                logging.info('QEI.testconn(). Masked URL: {0}'.format(masked_url))
    ...
    def fave_dashboards(self, user_id):
        ...
        for o in qry.all():
            ...
            if o.Dashboard.created_by:
                ...
                d['creator'] = str(user)
                d['creator_url'] = '/qei/profile/{}/'.format(
                    user.username)
            payload.append(d)
        return json_success(
            json.dumps(payload, default=utils.json_int_dttm_ser))
    ...
    def dashboard(self, dashboard_id):
        ...
        dash_edit_perm = check_ownership(dash, raise_if_false=False) and \
            security_manager.can_access('can_save_dash', 'QEI')
        dash_save_perm = security_manager.can_access('can_save_dash', 'QEI')
        superset_can_explore = security_manager.can_access('can_explore', 'QEI')
...
appbuilder.add_view_no_menu(QEI)
...
class CustomParameterModelView(SupersetModelView, DeleteMixin):
    datamodel = SQLAInterface(models.CustomParameters)
    list_columns = ['param_name','param_type','default_param_value','param_values','param_alias']
    search_columns = ['param_name','param_type','param_values','param_alias']
    edit_columns = ['param_name','param_type','default_param_value','param_values','param_alias']
    add_columns = edit_columns
    label_columns = {
        'param_name': _("Parameter Name"),
        'param_type': _("Parameter Type"),
        'default_param_value': _("Default Parameter Value"),
        'param_values': _("Parameter Value"),
        'param_alias': _("Parameter Alias"),
        
    }
    description_columns = {
        'param_name': _('Parameter name cannot contain \'{\' or \'}\' sign '),
        'param_type': _('Currently supported data types are: integers, decimals or strings.'),
        'default_param_value': _("Specify one value from the list above which will be the default parameter value."),
        'param_values': _("List of comma separated values (note: include default value as well)"),
        'param_alias': _('Enter aliases for parameter values (note: number of aliases should match number of parameters)')
    }

    def __isfloat(self, col):
        try:
            float(col)
        except ValueError:
            return False
    ...
    def pre_add(self, db):
        ...
        # this needs to be implemented with a template
        if '$' in db.param_name or '{' in db.param_name or '}' in db.param_name:
            raise ValueError('Error! Parameter name can not contain spaces,\{ or \} sign. Please check again!')
        db.param_values =  db.param_values.rstrip(',')
        if db.param_type.id == 2:
            if not db.default_param_value.isdigit():
                raise ValueError('Error! Please Enter an Integer value for the Default Parameter')
            if [i.isdigit() for i in db.param_values.split(',') if not i.isdigit()]:
                raise ValueError('Error! Please Enter Integer values for Parameter Values')

        if db.param_type.id == 3:
            if not self.__isfloat(db.default_param_value):
                raise ValueError('Error! Please Enter a Decimal value for the Default Parameter')
            if [self.__isfloat(i) for i in db.param_values.split(',') if not self.__isfloat(i)]:
                raise ValueError('Error! Please Enter Decimal values for Parameter Values')

        if not [i for i in db.param_values.split(',') if db.default_param_value == i]:
            raise ValueError('Error! Please add Default Value to Parameter Values')
        
        # validations for parameter alias
        if db.param_alias:
            if not len(db.param_alias.split(',')) == len(db.param_values.split(',')):
                raise ValueError('Error! Number of aliases and parameters should be same')
    ...
    def pre_update(self, db):
        ...
        if db.param_type.id == 2:
            if not db.default_param_value.isdigit():
                raise ValueError('Error! Please Enter an Integer value for Default Parameter')
            if [i.isdigit() for i in db.param_values.split(',') if not i.isdigit()]:
                raise ValueError('Error! Please Enter Integer values for Parameter Values')
...
class CustomParameterAsyncModelView(CustomParameterModelView):
    list_columns = ['param_name','param_type','default_param_value','param_values','param_alias']

appbuilder.add_view(
    CustomParameterModelView,
    "Parameters",
    #label=__("Superset"),
    icon="fa-code",
    category="Sources",
...
appbuilder.add_view_no_menu(CustomParameterAsyncModelView)
...
def panoramix(url):  # noqa
    return redirect(request.full_path.replace('panoramix', 'qei'))


@app.route('/<regex("caravel\/.*"):url>')
def caravel(url):  # noqa
    return redirect(request.full_path.replace('caravel', 'qei'))

@app.route('/<regex("superset\/.*"):url>')
def superset(url):  # noqa
    return redirect(request.full_path.replace('superset', 'qei'))
