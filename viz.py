...
class TableViz(BaseViz):
    ...
    def query_obj(self):
        ...
        if fd.get('all_columns'):
            ...
            sort_by_label = utils.get_metric_names(sort_by)
    ...
    def get_data(self, df):
        ...
        if len(percent_metrics):
            ...
            for m in percent_metrics:
                m_name = '%' + m
                df.loc[:, m_name] = pd.Series(metric_percents[m], index=df.index)
...
#Custom Bar Chart
class CustomLineBarViz(NVD3Viz):

    """A good old bar chart"""

    viz_type = 'line_string'
    verbose_name = _('Custom Line Chart')
    is_timeseries = False

    def query_obj(self):
        d = super(CustomLineBarViz, self).query_obj()  # noqa
        fd = self.form_data
...
class ParameterBoxViz(BaseViz):

    """A multi filter, multi-choice filter box to make dashboards interactive"""

    viz_type = 'parameter_box'
    ...
    def filter_query_obj(self):
        qry = super(ParameterBoxViz, self).query_obj()
        groupby = self.form_data.get('groupby')
        if len(groupby) < 1 and not self.form_data.get('date_filter'):
            raise Exception(_('Pick at least one filter field'))
        #qry['metrics'] = [self.form_data['metric']]
    ...
    def get_data(self, df):
        ...
        for flt in filters:
            df = self.dataframes[flt]
            df = df[df['param_name'] == flt]
            if df.param_alias.all():
                df = df.param_alias.tolist()
            else:
                df = df.param_values.tolist()
            try:
                df = df[0].split(',')
            except Exception:
                raise Exception('Error in specified parameter/s. Please check slice configurations')
            param_data[flt] = [{
                'id': row,
                'text': row,
                'filter': flt}
